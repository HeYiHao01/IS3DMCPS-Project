...
/**
 * wms_prd_out_detlEntity
 * @author xx
 * @version 2019-05-14
 */
@Table(name="wms_prd_out_detl", alias="a", columns={
		...
		@Column(name="prod_time", attrName="prodTime", label="投料出库时间"),
		@Column(name="lotnum", attrName="lotnum", label="投料批次"),
		@Column(name="equ_id", attrName="equId", label="投料喂丝机号"),
	}, orderBy="a.wo_no DESC"
)
public class WmsPrdOutDetl extends DataEntity<WmsPrdOutDetl> {
	...
	private Date prodTime;		// 投料出库时间
	private String lotnum;		// 投料批次
	private String equId;		// 投料喂丝机号
	
	public WmsPrdOutDetl() {
		this(null);
	}

	public WmsPrdOutDetl(String id){
		super(id);
	}
	...
	@Length(min=0, max=30, message="投料批次长度不能超过 30 个字符")
	...
	@Length(min=0, max=20, message="投料喂丝机号长度不能超过 20 个字符")
